...
public abstract class MediaAccessor {
	...
	public IMediaData[] reLoadMediaDataFromRMS(String recordName)
		...
		try {
			...
			while (isEnum.hasNextElement()) {
				...
				Object media = getMediaInfoTable().get(label);
				if( media == null )
					getMediaInfoTable().put(label, iiObject);
			}
			infoStore.closeRecordStore();
	...
	public IMediaData getMediaInfo(String imageName) throws ImageNotFoundException {	
		IMediaData ii = (IMediaData) getMediaInfoTable().get(imageName);
	 ...
	 */
	public void setMediaInfo(String mediaName, IMediaData newData) {	
	...
	public boolean deleteSingleMediaFromRMS(String storeName, String mediaName) throws PersistenceMechanismException, ImageNotFoundException {
		...
		try {
			...
			IMediaData mediaData = getMediaInfo(mediaName);
			int rid = mediaData.getForeignRecordId();
			
			System.out.println(album_label + storeName + ":"+info_label + storeName +":" + rid);
